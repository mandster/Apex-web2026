...
const ProductDetail = ({ initialId = null }) => {
    ...
    <main className="page-shell">
      ...
      <section className="page-section product-story-grid">
        <article className="variant-card-editorial wide-variant-card">
          <img src={image.src || image} alt={`${name} for industrial motor control panel use`} className="variant-image" loading="lazy" />
        </article>
      </section>
      <section className="page-section">
        <div className="suggestion-buttons editorial-chip-row">
          {productsData
            .filter((p) => p.id !== id)
            .map((suggestion) => (
              <Link key={suggestion.id} className="rounded-btn editorial-chip" href={`/ProductDetail/${suggestion.id}`}>
                {suggestion.name}
              </Link>
            ))}
        </div>
